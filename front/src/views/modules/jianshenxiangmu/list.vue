  ...
  <div class="main-content">
    ...
    <div class="page-header">
      <div class="header-left">
        ...
        <div class="page-subtitle">管理健身房健身项目信息</div>
      </div>
    ...
    <div v-if="showFlag">
        ...
        <el-collapse-transition>
          <el-form v-show="searchExpanded" :inline="true" :model="searchForm" class="form-content">
            <div class="search-row">
              ...
              <div class="search-field">
                <label class="search-label">收费标准</label>
                <el-input 
                  v-model="searchForm.shoufeibiaozhun" 
                  placeholder="请输入收费标准"
                  clearable
      ...
      <div class="action-bar">
        <div class="action-bar-left">
          <el-button v-if="isAuth('jianshenxiangmu','新增')" type="primary" icon="el-icon-plus" class="action-btn action-btn-add" @click="addOrUpdateHandler()">
            <span class="btn-text">新增项目</span>
          </el-button>
          <el-button 
            v-if="isAuth('jianshenxiangmu','删除')"
      ...
      <div class="table-card">
        ...
        >
          ...
          <el-table-column prop="xiangmufengmian" header-align="center" align="center" label="项目封面" min-width="120">
            <template slot-scope="scope">
              <div v-if="scope.row.xiangmufengmian" class="cell-image">
                <el-image 
                  :src="scope.row.xiangmufengmian.split(',')[0]" 
                  fit="cover"
                  :preview-src-list="scope.row.xiangmufengmian.split(',')"
                  class="project-image"
                >
          ...
          <el-table-column prop="xiangmushipin" header-align="center" align="center" label="项目视频" min-width="120">
            <template slot-scope="scope">
              <div v-if="scope.row.xiangmushipin" class="cell-video">
                <el-button type="text" icon="el-icon-video-play" class="video-btn play-btn" @click="previewVideo(scope.row.xiangmushipin)">播放</el-button>
                <el-button type="text" icon="el-icon-download" class="video-btn download-btn" @click="downloadVideo(scope.row.xiangmushipin)">下载</el-button>
              </div>
          ...
          <el-table-column width="280" align="center" fixed="right" label="操作" class-name="action-column">
            <template slot-scope="scope">
              <div class="action-buttons">
                ...
                <el-button v-if="isAuth('jianshenxiangmu','修改')" type="text" icon="el-icon-edit" class="action-link action-edit" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                <el-button v-if="$storage.get('sessionTable') == 'users' && isAuth('jianshenxiangmu','修改')" type="text" icon="el-icon-guide" class="action-link action-guide" @click="zhidaoxiangmuCrossAddOrUpdateHandler(scope.row,'cross')">指导</el-button>
                <el-button v-if="isAuth('jianshenxiangmu','删除')" type="text" icon="el-icon-delete" class="action-link action-del" @click="deleteHandler(scope.row.id)">删除</el-button>
              </div>
            </template>
          </el-table-column>
        </el-table>
    ...
    <!-- 视频预览弹窗 -->
    <el-dialog title="视频预览" :visible.sync="videoPreviewVisible" width="60%" class="video-dialog" :close-on-click-modal="false">
      <div class="video-container">
        <video v-if="videoPreviewVisible" :src="currentVideoUrl" controls class="video-player">您的浏览器不支持 video 标签</video>
      </div>
      <span slot="footer" class="dialog-footer">
        <el-button @click="videoPreviewVisible = false" class="dialog-close-btn">关闭</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getDataList() {
      ...
      if(this.searchForm.shoufeibiaozhun) {
    ...
    previewVideo(file) {
      ...
      // 处理视频URL，拼接完整的后端地址
      // 视频存放在后端的 video 文件夹，通过 /video/{filename} 访问
      if (file.startsWith('http') || file.startsWith('//')) {
        this.currentVideoUrl = file;
      } else {
        // 从路径中提取文件名，如 "D:/gitcode/.../1.mp4" 或 "1.mp4" 或 "video/1.mp4"
        const fileName = file.split(/[/\\]/).pop();
        this.currentVideoUrl = 'http://localhost:8080/springboot97s0t/video/' + fileName;
      }
    ...
    downloadVideo(file) {
      ...
      const fileName = file.split(/[/\\]/).pop();
      if (!fileName) {
        this.$message.warning('无法获取文件名');
        return;
      }
      this.$http({
        url: '/jianshenxiangmu/download',
        method: 'get',
        params: { fileName },
        responseType: 'blob'
      }).then(res => {
    ...
    deleteHandler(id) {
      ...
      this.$confirm(`确定进行[${id ? "删除" : "批量删除"}]操作?`, "提示", {
        ...
        this.$http({
          url: "jianshenxiangmu/delete", method: "post", data: ids
        }).then(({ data }) => {
          if (data && data.code === 0) {
