    ...
    <div v-if="showFlag">
      ...
      <div class="page-header">
        <div class="header-left">
          ...
          <div class="page-subtitle">管理健身项目指导教练信息</div>
        </div>
      ...
      <div class="action-bar">
        <div class="action-bar-left">
          <el-button v-if="$storage.get('sessionTable') == 'users'" type="primary" icon="el-icon-plus" class="action-btn action-btn-add" @click="addOrUpdateHandler()">
            <span class="btn-text">新增指导</span>
          </el-button>
          <el-button v-if="$storage.get('sessionTable') == 'users'"
          ...
          <el-button v-if="$storage.get('sessionTable') == 'yuangong'" type="success" icon="el-icon-user" class="action-btn action-btn-mine" @click="goToMyZhidao()">
            <span class="btn-text">我的指导项目</span>
          </el-button>
        </div>
      ...
      <div class="table-card">
        ...
        >
          ...
          <el-table-column prop="zhidaojiaolian" header-align="center" align="center" label="指导教练" min-width="100">
            <template slot-scope="scope">
              <div class="cell-coach">
                <el-avatar :size="32" class="coach-avatar">{{ (scope.row.zhidaojiaolian || '-').substring(0, 1) }}</el-avatar>
                <span class="coach-name">{{ scope.row.zhidaojiaolian || '-' }}</span>
              </div>
            </template>
          </el-table-column>
          <el-table-column width="200" align="center" fixed="right" label="操作" class-name="action-column">
            <template slot-scope="scope">
              <div class="action-buttons">
                ...
                <el-button v-if="$storage.get('sessionTable') == 'users'" type="text" icon="el-icon-edit" class="action-link action-edit" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                <el-button v-if="$storage.get('sessionTable') == 'users'" type="text" icon="el-icon-delete" class="action-link action-del" @click="deleteHandler(scope.row.id)">删除</el-button>
              </div>
            </template>
          </el-table-column>
        </el-table>
...
export default {
  ...
  methods: {
    ...
    goToMyZhidao() {
      this.$router.push({ path: '/zhidaoxiangmu/myZhidao' });
    }
  }
};
</script>
